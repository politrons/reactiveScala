...
class ResolverThreeLevelsTest extends FlatSpec with Matchers with BeforeAndAfterAll with Logging {
  ...
  it should "on query, first send it to the master node, and then to other configured nodes" in {
    ...
    lazy val normalNodeSvc = Service.mk[Request, Response] { _: Request => {
      ...
      rep.setContentString(
        s"""
            {"instances":[
              {"host":"127.0.0.1","port":$normalPort1,"datacenter":"normal","confidence":100}
           |            ]}
        """.stripMargin
      )
      Future.value(rep)
